...
//     public async getContentBasedRecommendations(userId: string, limit: number = 1000): Promise<Book[]> {
//         const user = await this.userRepository.findById(userId);
//         if (!user) throw new Error('User not found');

//         const allBooks = await this.bookRepository.findAllAuthenticate(userId,limit, 0);
...
export class BookRecommendationService {
    ...
    public async getContentBasedRecommendations(userId: string, limit: number = 1000): Promise<Book[]> {
        ...
        // Compute user vector and boost user preference terms by factor 2
        const userVector = this.computeTFIDFVector(userProfileWords.join(" "), corpus);
        for (const term of userVector.keys()) {
            userVector.set(term, userVector.get(term)! * 2);
        }

        const scoredBooks = allBooks.map(book => {
            const bookText = this.getBookText(book);
            const bookVector = this.computeTFIDFVector(bookText, corpus);
            let similarity = this.calculateCosineSimilarity(userVector, bookVector) * 10;

            // Boost score if book genre matches any user preference (case insensitive)
            if (user.preferences?.some(pref => pref.toLowerCase() === book.genre?.toLowerCase())) {
                similarity += 5;
            }
        ...
        // Filter out low similarity scores (< 0.2)
        const filteredBooks = scoredBooks.filter(({ score }) => score > 0.2);

        return this.getTopScoredBooks(filteredBooks, limit);
    }
